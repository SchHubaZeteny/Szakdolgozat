...
public class PortalTwoController : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.CompareTag("PlayerPickUpHitbox") && !controller.portalOnCooldown)
        {
            ...
            controller.portalTwoEntered = true;
            player.GetComponent<PlayerController>().isPortaling = true;
        }

        if (other.CompareTag("PlayerPickUpHitbox") && controller.portalOnCooldown && controller.portalOneEntered)
        {
            controller.portalOneEntered = false;
